...
export default function Dashboard() {
  ...
  const donutData = [
    ...
    { label: "Escalated", value: a.escalated,     color: "#A100FF" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg, #0d0f14 0%, #110820 100%)", border: "1px solid #A100FF20" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <h1 className="text-lg font-bold text-white leading-snug">
                Insurance fraud costs <span style={{ color: "#ef4444" }}>$3.1B/year</span> in India.
                <br />Claims take <span style={{ color: "#f59e0b" }}>21+ days</span> to settle.
              </h1>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {[
                  ...
                  <div key={s.label} className="px-3 py-1.5 rounded-lg" style={{ background: "#ffffff08", border: "1px solid #ffffff08" }}>
                    <div className="text-sm font-bold text-white tabular-nums">{s.n}</div>
                    <div className="text-[9px] text-[#475569]">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col justify-between">
              <div>
                <div className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "#A100FF" }}>
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={f.text} className="flex items-center gap-2 text-xs text-[#64748b]">
        ...
        {/* Pipeline strip */}
        <div className="border-t px-6 py-3 flex items-center gap-0 overflow-x-auto" style={{ borderColor: "#A100FF15", background: "#08050f" }}>
          <span className="text-[9px] font-semibold text-[#334155] uppercase tracking-wider shrink-0 mr-3">AI Pipeline:</span>
          {["Language Agent", "Policy Match", "Fraud Analysis", "Ring Detection", "Decision + Audit"].map((s, i, arr) => (
            <div key={s} className="flex items-center shrink-0">
              ...
              {i < arr.length - 1 && (
                <svg className="w-3 h-3 text-[#1e2a3a] mx-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                </svg>
              )}
            </div>
          ))}
          <span className="ml-auto text-[10px] font-semibold shrink-0" style={{ color: "#A100FF" }}>⚡ All in &lt;3 seconds</span>
        </div>
      </div>
      ...
      {/* ── Impact numbers ─────────────────────────────────────────────────── */}
      <div>
        <div className="text-[10px] font-semibold text-[#1e2a3a] uppercase tracking-wider mb-3">What Our AI Has Done So Far</div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex-1 space-y-2">
              {donutData.map((d) => (
                <div key={d.label} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs text-[#64748b] flex-1">{d.label}</span>
                  <span className="text-xs font-bold tabular-nums text-white">{d.value}</span>
                  <span className="text-[10px] text-[#334155] tabular-nums w-8 text-right">
          ...
          <div className="surface rounded-xl px-5 py-4">
            ...
            <div className="text-xs text-[#64748b] mt-2">Fraud Exposure Blocked</div>
            <div className="text-[10px] text-[#334155] mt-0.5">{a.flagged + a.escalated} claims flagged or escalated</div>
          </div>

          {/* Speed */}
          <div className="surface rounded-xl px-5 py-4">
            <div className="flex items-center gap-2">
              <div className="text-xl font-bold tabular-nums leading-none" style={{ color: "#A100FF" }}>&lt;3s</div>
              <div className="text-[10px] text-[#334155] leading-tight">vs<br/>21 days</div>
            </div>
            <div className="text-xs text-[#64748b] mt-2">AI Processing Speed</div>
            <div className="flex items-center gap-1 mt-1">
              <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#A100FF" }} />
              <span className="text-[10px]" style={{ color: "#A100FF70" }}>vs 21-day industry avg</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          <div key={kpi.label} className="surface rounded-xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-[10px] text-[#334155] mt-0.5">{kpi.sub}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="surface rounded-xl p-5">
        <div className="text-[10px] font-semibold text-[#334155] uppercase tracking-wider mb-4">Follow the Demo — 6 Chapters</div>
          ...
          {[
            ...
            <Link key={step.n} href={step.href}
              className={`flex flex-col gap-1 rounded-xl px-3 py-3 transition-all border ${
                step.active
                  ? "border-[#A100FF]/30 text-white"
                  : "border-white/[0.04] text-[#4b5563] hover:text-[#94a3b8] hover:border-white/[0.08]"
              }`}
              style={step.active ? { background: "#A100FF10" } : {}}>
              <span className="text-[9px] font-bold" style={step.active ? { color: "#A100FF" } : { color: "#1e2a3a" }}>
                STEP {step.n}
              </span>
              <span className="text-[11px] font-semibold leading-tight">{step.title}</span>
              <span className="text-[9px] leading-tight" style={step.active ? { color: "#A100FF60" } : { color: "#1e2a3a" }}>
                {step.sub}
              </span>
            </Link>
          ))}
      ...
      <div>
        <div className="text-[10px] font-semibold text-[#1e2a3a] uppercase tracking-wider mb-3">Live Evidence — Most Recent Claims</div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs text-[#334155]">Each row = one AI decision made in &lt;3 seconds</p>
              <Link href="/claims" className="text-xs text-[#64748b] hover:text-white transition-colors">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {claims.slice(0, 6).map((c) => (
                    <tr key={c.claim_id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-[#A100FF] font-medium">{c.claim_id}</td>
                      <td className="px-4 py-3 text-[#e2e8f0] font-medium">{c.claimant_name}</td>
                      <td className="px-4 py-3 text-[#64748b] capitalize text-xs">{c.claim_type.replace(/_/g, " ")}</td>
                      <td className="px-4 py-3 text-right text-[#e2e8f0] font-semibold tabular-nums">${c.amount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-center"><StatusBadge status={c.status} /></td>
                    </tr>
                  ))}
                  {claims.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-4 py-10 text-center text-[#334155] text-xs">
                        No claims yet.{" "}
                        <Link href="/submit" className="text-[#A100FF] hover:underline">Submit one to start the demo →</Link>
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div>
            <p className="text-xs text-[#334155] mb-2">Claims processing in real time</p>
            <LiveFeed />
          </div>
        </div>
      </div>
...
function DonutChart({ data, total }: { data: { value: number; color: string }[]; total: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
      ...
      <text x={cx} y={cy - 5} textAnchor="middle" fontSize="16" fontWeight="bold" fill="white">{total}</text>
      <text x={cx} y={cy + 10} textAnchor="middle" fontSize="7" fill="#64748b">PROCESSED</text>
    </svg>
  );
}
...
function InsightStrip({ a, rings, total }: { a: Analytics; rings: FraudRing[]; total: number }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl surface border border-[#A100FF]/10">
      <span className="relative flex h-1.5 w-1.5 shrink-0">
        <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-50" style={{ background: "#A100FF" }} />
        <span className="relative inline-flex rounded-full h-1.5 w-1.5" style={{ background: "#A100FF" }} />
      </span>
      <span className="text-[11px] font-semibold" style={{ color: "#b84dff" }}>Live AI Insight</span>
...
function Th({ children, align = "left" }: { children: React.ReactNode; align?: "left" | "right" | "center" }) {
  return (
    <th className={`px-4 py-2.5 text-[10px] font-semibold text-[#334155] uppercase tracking-wider text-${align}`}>
      {children}
    </th>
  );
}
  ...
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="col-span-2 h-28 rounded-xl bg-white/[0.04]" />
        {Array(2).fill(0).map((_, i) => <div key={i} className="h-28 rounded-xl bg-white/[0.04]" />)}
      </div>
      <div className="h-28 rounded-xl bg-white/[0.04]" />
      <div className="h-10 rounded-xl bg-white/[0.03]" />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        <div className="lg:col-span-2 h-64 rounded-xl bg-white/[0.04]" />
