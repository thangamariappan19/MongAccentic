...
export default function ClaimsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Chapter card */}
      <div className="rounded-xl px-5 py-4" style={{ background: "#0a0d12", border: "1px solid #6C63FF20" }}>
        ...
        <h1 className="text-base font-bold text-[#223A66]">Every row = one AI decision, made in under 3 seconds</h1>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            { color: "#6C63FF", label: "ESCALATED",desc: "Senior adjuster required" },
            ...
            <div key={s.label} className="flex items-center gap-1.5 text-[10px]">
              ...
              <span className="text-[#64748b]">— {s.desc}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-[#223A66]">All Claims</h2>
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3">
        <div className="relative">
          ...
          <input
            ...
            className="surface rounded-lg pl-9 pr-4 py-2 text-sm text-[#223A66] placeholder-[#94a3b8] focus:outline-none w-full sm:w-56"
          />
        </div>
          ...
          {FILTERS.map((f) => (
            <button key={f.value} onClick={() => setFilter(f.value)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${
                filter === f.value
                  ? "border-[#6C63FF]/40 bg-[#6C63FF]/10 text-[#b84dff]"
                  : "border-[#e2e8f0] bg-transparent text-[#64748b] hover:text-[#6C63FF] hover:border-[#6C63FF]/40"
              }`}>
              {f.label}
            </button>
          ))}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.05]">
                <th className="px-5 py-3 text-left text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Claim ID</th>
                <th className="px-5 py-3 text-left text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Claimant</th>
                <th className="px-5 py-3 text-left text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Type</th>
                <th className="px-5 py-3 text-right text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Amount</th>
                <th className="px-5 py-3 text-center text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Fraud</th>
                <th className="px-5 py-3 text-center text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Status</th>
                <th className="hidden sm:table-cell px-5 py-3 text-left text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">Date</th>
                <th />
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.03]">
              {paginated.map((c) => (
                <tr key={c.claim_id} className="hover:bg-[#faf5ff] transition-colors group">
                  <td className="px-5 py-3.5 font-mono text-xs text-[#6C63FF] font-medium">{c.claim_id}</td>
                  ...
                  <td className="hidden sm:table-cell px-5 py-3.5 text-xs text-[#64748b] tabular-nums">
                    {new Date(c.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                  </td>
                  <td className="px-5 py-3.5 text-right">
                    <Link href={`/claims/${c.claim_id}`}
                      className="text-xs text-[#64748b] group-hover:text-[#6C63FF] transition-colors font-medium">
                      View →
                    </Link>
                  </td>
                </tr>
              ...
              {visible.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-5 py-10 text-center text-xs text-[#64748b]">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={() => setPage(0)}
              disabled={page === 0}
              className="px-2.5 py-1.5 rounded-lg text-xs surface border border-[#e2e8f0] text-[#64748b] disabled:opacity-30 hover:text-[#6C63FF] transition-colors">
              «
            </button>
            <button
              onClick={() => setPage((p) => Math.max(0, p - 1))}
              disabled={page === 0}
              className="px-3 py-1.5 rounded-lg text-xs surface border border-[#e2e8f0] text-[#64748b] disabled:opacity-30 hover:text-[#6C63FF] transition-colors">
              Prev
            </button>
              ...
              return (
                <button key={pageNum} onClick={() => setPage(pageNum)}
                  className={`w-8 h-8 rounded-lg text-xs font-medium transition-colors ${
                    pageNum === page
                      ? "bg-[#6C63FF]/20 text-[#b84dff] border border-[#6C63FF]/30"
                      : "surface border border-[#e2e8f0] text-[#64748b] hover:text-[#6C63FF]"
                  }`}>
                  {pageNum + 1}
                </button>
              );
...
function FraudScore({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      <div className="w-10 h-1 rounded-full bg-[#f1f5f9] overflow-hidden">
